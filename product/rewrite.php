...
$num=$_GET['num'];
	...
	foreach($num as $p){
		...
		$que2="select num from ptest where pid='".$p."'";//기존제품제거
		$result2 = $mysqli->query($que2) or die("1:".$mysqli->error);
		$rs2 = $result2->fetch_array();
		$idx=$rs2[0];
		

		$que="update product set IS_DELETE='1' where PRODUCT_ID='".$p."'";//상태값만 변경
		$sql=$mysqli->query($que) or die($mysqli->error);

		$que="update ptest set gubun='0' where pid='".$p."'";//상태값만 변경
		$sql=$mysqli->query($que) or die($mysqli->error);

		$que3="select FILE_ORDER,PRODUCT_ID,FILEPATH,FILENM_SYS,REP_FLAG from product_file_info where PRODUCT_ID='".$p."'";//기존제품제거
		...
		$que="select * from ptest where num='".$idx."'";
		$result = $mysqli->query($que) or die("38:".$mysqli->error);
