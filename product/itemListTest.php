...
//	}
	...
	$que2="select count(1) from taobao  where uid='$uid' and price is not Null and isDisplay='1' $where";
	$que2.=$where;
       ...
       <dd>
		...
		<div class="midle_bu_area ml20">
			<ul class="left_bu_area">
				...
				<li><button type="button" id='reget' val='reget' class="button03_4" style="width:150px;height:30px;padding:4px 20px;">데이타다시받기</button></li>
...
$no=$total-($page-1)*$ps;//번호매기기
	...
	$que="select * from taobao   where uid='$uid' and price is not Null and isDisplay='1'";
	$que.=$where;
	$que.=$order;
	$que.=$limit_query;
//	echo $que;
...
?>
				...
				?>
						<?if($cnp1<$cn1){?>
							<br><font color="red">(할인가: <?=$p->promoPrice?>위안 / 약 <?echo number_format($cnp1*$cny)?>원)</font>
						<?}?>
					  </td>
					  <td>
						<?if($p->optionCount>2){?>
							옵션이 세개이상인 상품은 직접 옵션을 등록하셔야합니다
						<?}else{?>
						<?php
							...
							$optionValue="";
							if($rs2->optionValue1)$optionValue=$rs2->optionValue1."<br>";
							if($rs2->optionValue2)$optionValue.=$rs2->optionValue2."<br>";
							//if($rs2->optionValue3)$optionValue.=$rs2->optionValue3."<br>";
							echo $optionValue;

							$jd1=urldecode($rs2->optionPromoPrice);
							$jd=json_decode(str_replace("'","\"",$jd1));
							//echo "<pre>";
							//print_r($jd);
