...
?>
	...
	<br>
	(※ 옵션이 3개 이상인 상품은 옵션을 직접 등록하셔야합니다.<br>옵션이미지도 사용할 수 없습니다.)
...
?>
						...
						<ul>
							...
							<li><span style="font-weight:bold;">판매가</span> : <input type="text" numberOnly name="myPrice" id="myPrice" <?if($p->optionType==2){?>readonly alt="opt" onclick="alert('옵션가수정 버튼을 눌러 수정하세요.');"<?}?> value="<?=$p->myPrice?>" style="width:80px;text-align:right;">원</li>
						...
						if($p->optionType==2){
							...
							if($p->optionCount>3 and $p->myPrice==0){
					  ?>
					...
					<?}else{?>
					  ...
					  <td style="text-align:center;line-height:25px;">
						<a href="javascript:;" onclick="window.open('/product/myItemView.php?num=<?=$p->myNum?>','iv','width=900,height=800,scrollbars=yes')">미리보기</a>
						<br>
...
function optionPrice(n){
	window.open('/product/optionPriceEditMulti.php?num='+n,'pte','width=500,height=640,scrollbars=yes');
}
...
//			console.log(this.value);
	...
	$('textarea[id="optCont3"]').each(function() {
			optContArray3[total_cnt]=this.value;//배열로 저장
			total_cnt++;
	});
