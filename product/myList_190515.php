      ...
      <dl>
	   ...
	   <div class="midle_bu_area ml20">
		  <ul class="left_bu_area">
			...
			<li style="margin-left:0px;margin-right:0px;"><a class="button03_4" href="javascript:;" id='excelSave' style="padding:5px 10px;"><img src="/images/excel_ico01.png" alt="Excel">엑셀로저장</a></li>
		</ul>
		 </div>
                ...
                <table width="100%" border="0" style="table-layout:fixed; word-break:break-all;">
                  ...
                  <thead>
                    <tr>
					  ...
					  <th scope="col">배송방법/배송비유형</th>
                      <th scope="col">배송비설정 <!-- <a href="javascript:;" onclick="allSend();">▼</a> --></th>
					  <th scope="col" style="width:450px;">옵션<br><font color="red">※옵션과 옵션은 , 로 구분합니다.<!-- <br>빼고싶은 옵션은 옵션가수정을 클릭후 해당옵션의 가격을 -1000으로 수정하십시오. --></font> 
					  </th>
...
?>
                    <tr style="overflow: hidden;">
					  ...
					  <td style="text-align:center;"><img src="/thumb/<?echo $tf[0]?>" width="100"></td>
                      <td style="text-align:left;line-height:25px;">
							<table>
							<tr>
								<td style="width:50%;background-color:#fff;">
									<textarea name="itemName_<?=$p->num?>" id="itemName" placeholder="상품명을 입력하세요." style="width:280px;height:60px;"><?echo stripslashes($p->itemName);?></textarea>
								</td>
							</tr>
							<tr>
					  ...
					  <td style="text-align:center;line-height:25px;">
						<a href="javascript:;" onclick="window.open('/product/myItemView.php?num=<?=$p->myNum?>','iv','width=900,height=800,scrollbars=yes')">보기</a>
						<br>
						<a href="javascript:;" onclick="window.open('/product/myItemEdit.php?num=<?=$p->myNum?>','iv','width=900,height=800,scrollbars=yes')">편집</a>
					  </td>
...
?>
						...
						<ul>
							<li><span style="font-weight:bold;">타오바오</span> : <?echo number_format($itemPrice);?>위안</li>
							<li><span style="font-weight:bold;">환율가</span> : <?echo $exPrice;?></li>
							<li><span style="font-weight:bold;">판매가</span> : <input type="text" name="myPrice" id="myPrice" <?if($p->optionType==2){?>readonly alt="opt"<?}?> value="<?=$p->myPrice?>" style="width:100px;text-align:right;">원</li>
							<li><span style="font-weight:bold;color:red">할인설정</span> : <input type="text" size="3" style="text-align:right;" name="nowFee" id="nowFee" value="<?=$p->nowFee?>">
					  ...
					  <td>
							...
							<br><br>
							<?php
							$sendFeeType=$p->sendFeeType?$p->sendFeeType:"조건부 무료";
					  ...
					  ?>
							<select name="sendFeeType" id="sendFeeType" onchange="selcon(this.value,'<?=$p->num?>')">
								<option value="조건부 무료" <?if($p->sendFeeType=="조건부 무료"){?>selected<?}?>>조건부 무료</option>
								<option value="수량별" <?if($p->sendFeeType=="수량별"){?>selected<?}?>>수량별</option>
								<option value="유료" <?if($p->sendFeeType=="유료"){?>selected<?}?>>유료</option>
								<option value="무료" <?if($p->sendFeeType=="무료"){?>selected<?}?>>무료</option>
							</select>
...
<?if($p->optionCount>2){?>
	...
	<br>
	(※ 옵션이 3개 이상인 상품은 옵션을 직접 등록하셔야합니다.)
<?}else{?>
...
<?php
						...
						<table>
							<tr>
								...
								<td rowspan="2" style="width:50%">
									<textarea name="optCont1_<?=$p->num?>" id="optCont1" style="width:250px;height:90px;" ><?echo $rs2->optionValue1;?></textarea>
								</td>
							</tr>
							<tr>
								<td style="font-size:8px;">
					  ...
					  <?}else{?>
							<textarea name="optCont1" id="optCont1" style="width:200px;height:60px;display:none" ><?echo $rs2->optionValue1;?></textarea>
					  <?}?>
					  ...
					  ?>
						  <table>
							<tr>
								...
								<td rowspan="2" style="width:50%">
									<textarea name="optCont2_<?=$p->num?>" id="optCont2" style="width:250px;height:90px;" ><?echo $rs2->optionValue2;?></textarea>
								</td>
							</tr>
							<tr>
								<td style="font-size:8px;">
					  ...
					  ?>
					   ...
					   <?}else{?>
							<textarea name="optCont2" id="optCont2" style="width:200px;height:60px;display:none" ><?echo $rs2->optionValue2;?></textarea>
					  <?}?>
<?}?>

					  </td>
					  <td style="line-height:30px;">
						<!-- <select name="optionType" id="optionType" onchange="otchange(this.value,'<?=$p->num?>')">
							<option value="0" <?if(!$p->optionType){?>selected<?}?>>옵션없음</option>
							<option value="1" <?if($p->optionType==1){?>selected<?}?>>단독형</option>
							<option value="2" <?if($p->optionType==2){?>selected<?}?>>조합형</option>
						</select> -->
						<?if($p->optionType==0){?>옵션없음<?}else if($p->optionType==1){?>단독형<?}else if($p->optionType==2){?>조합형<?}?>
						<?php
						if($p->optionType==2){
					  ?>
						<br>
						...
						<?}else{?>
						<br>
						<button type="button" class="button03_4" id="optButton_<?=$p->num?>" style="display:none;" onclick="alert('먼저 수정한 내용을 저장하십시오.')">옵션가수정</button>
						<?}?>
					  </td>
                    </tr>
<?
...
<?}?>
    ...
    <!-- GRID E-->
	   ...
	   <div class="midle_bu_area ml20">
		  ...
		  <ul class="left_bu_area" style="padding-top:10px;">
			<li style="vertical-align:text-bottom;">선택한 상품을 </li>
			<li style="margin-left:0px;margin-right:0px;"><a class="button03_4" href="javascript:;" id='myDel2' style="padding:5px 10px;">삭제</a></li>
			<li style="margin-left:0px;margin-right:0px;"><a class="button03_4" href="javascript:;" id='myCate2' style="padding:5px 10px;">카테고리선택</a></li>
			<li style="margin-left:0px;margin-right:0px;"><a class="button03_4" href="javascript:;" id='zipSave2' style="padding:5px 10px;">대표이미지 ZIP로저장</a></li>
			<li style="margin-left:0px;margin-right:0px;"><a class="button03_4" href="javascript:;" id='zipSaveView2' style="padding:5px 10px;">상세이미지 ZIP로저장</a></li>
			<li style="margin-left:0px;margin-right:0px;"><a class="button03_4" href="javascript:;" id='excelSave2' style="padding:5px 10px;"><img src="/images/excel_ico01.png" alt="Excel">엑셀로저장</a></li>
			</ul>
			<ul class="right_bu_area" style="padding-top:10px;">
				<button type="button"  class="button03_5" id='mySave2' style="width:120px;height:35px;">수정내용저장</button>
			</ul>
			</div>
<!-- page_skip -->
...
function optionRegist(n){
	window.open('/product/optionRegist.php?num='+n,'pte','width=500,height=640,scrollbars=yes');
}
...
//			console.log(this.value);
	...
	$('textarea[id="optCont1"]').each(function() {
			optContArray1[total_cnt]=this.value;//배열로 저장
			total_cnt++;
	});
	...
	$('textarea[id="optCont2"]').each(function() {
			optContArray2[total_cnt]=this.value;//배열로 저장
			total_cnt++;
	});
