...
<!-- 메인메뉴 S -->
    <div id="wrap_tmall_content">
     ...
     <ul class="top_title_area">
       <li class="top_title">회원관리 > 소매사업자</li>
     </ul>
...
<input type="hidden" name="sq" id="sq"  value="<?=$sq?>">
   	       ...
   	       <table>
						   ...
						   <tr>
                               ...
                               <td>
									<select name="s_key">
										<option value="RETAILER_ID" <?if($s_key=="RETAILER_ID"){?> selected<?}?>>아이디</option>
										<option value="RNAME" <?if($s_key=="RNAME"){?> selected<?}?>>매장명</option>
										<option value="RETAILER_NAME" <?if($s_key=="RETAILER_NAME"){?> selected<?}?>>대표자</option>
							 ...
							 <tr>
                               <td scope="col" class="title1">회원상태</td>
                               <td>
									<input type="radio" name="STS_CODE" value="all" <?if($STS_CODE=="all"){?>checked<?}?>>전체&nbsp;
									<input type="radio" name="STS_CODE" value="0" <?if($STS_CODE=="0"){?>checked<?}?>>사용중&nbsp;
									<input type="radio" name="STS_CODE" value="1" <?if($STS_CODE=="1"){?>checked<?}?>>사용안함&nbsp;
                              </td>
							  <td align="right">
								<div class="midle_bu_area ml20">
...
<?
	$que3="select count(1) from retailer where AUTH_CODE='1'";
	$result3 = $mysqli->query($que3) or die("1:".$mysqli->error);
	$rs3 = $result3->fetch_array();
	$total_retailer=$rs3[0];
	...
	if($mode=="up" and $start_date and $end_date){
		$where.=" and UPDATE_DATE between '".$start_date."' and '".$end_date." 23:59:59'";
	}

	if($STS_CODE!="all"){
		$where.=" and STS_CODE='".$STS_CODE."'";
	}


	if(!$order){
		$order=" order by RETAILER_CODE desc";
	}
	...
	$que2="select count(1) from retailer where AUTH_CODE='1'";
	$que2.=$where;
...
?>
      ...
      <dl>
       <dt>총  회원수 <?echo number_format($total_retailer);?>명  |  검색결과    <?echo number_format($total);?>건</dt>
       <dd>
	   </dd>
      </dl>
      </div>
...
$no=$total-($page-1)*$ps;//번호매기기
	...
	$que="select * 
	from retailer where AUTH_CODE='1'";
	$que.=$where;
	$que.=$order;
	$que.=$limit_query;
//	echo $que;
...
?>
                    <tr>
                      <td><?=$no?></td>
                      <td><?=$p->REG_DATE?></td>
                      <td class="pl20"><a href="javascript:;" onclick="window.open('retailer_view.php?RETAILER_CODE=<?=$p->RETAILER_CODE?>','s1','width=900,height=800,scrollbars=yes')"><?=$p->RETAILER_ID?></a></td>
                      <td><?=$p->RNAME?></td>
                      <td><?=$p->RETAILER_NAME?></td>
                      <td><?=$p->BUSINESS_REGISTERED_NUMBER?></td>
					  <td><?=$p->CELLPHONE?></td>
...
<div class="page_skip_area">
	<div class="page_skip">
		<?if($f_no!=1){?><a href="<?=$PHP_SELF?>?mode=<?=$mode?>&page=<?=$p_f_no?>&f_no=<?=$p_f_no?>&multi=<?=$multi?>&ord=<?=$ord?>&s_key=<?=$s_key?>&s_word=<?=$s_word?>&start_date=<?=$start_date?>&end_date=<?=$end_date?>&STS_CODE=<?=$STS_CODE?>&spa=<?=$spa?>&m1=<?=$m1?>&m2=<?=$m2?>&m3=<?=$m3?>"><img src="/admin_page/images/btn_prev02.gif" alt="이전 목록" class="btn01" /></a><?}?>

				<? for($i=$f_no;$i<=$l_no;$i++){?>
					<?if($i==$page){?>
						<strong><?=$i?></strong>&nbsp;
					<?} else {?>
						<a href="<?=$PHP_SELF?>?mode=<?=$mode?>&page=<?=$i?>&f_no=<?=$f_no?>&multi=<?=$multi?>&sort=<?=$sort?>&s_key=<?=$s_key?>&s_word=<?=$s_word?>&start_date=<?=$start_date?>&end_date=<?=$end_date?>&STS_CODE=<?=$STS_CODE?>&spa=<?=$spa?>&m1=<?=$m1?>&m2=<?=$m2?>&m3=<?=$m3?>">
						<?=$i?>&nbsp;</a>
					<?}?>
				<?}?>

		<?if($l_no<$total_page){?><a href="<?=$PHP_SELF?>?mode=<?=$mode?>&page=<?=$n_f_no?>&f_no=<?=$n_f_no?>&multi=<?=$multi?>&ord=<?=$ord?>&s_key=<?=$s_key?>&s_word=<?=$s_word?>&start_date=<?=$start_date?>&end_date=<?=$end_date?>&STS_CODE=<?=$STS_CODE?>&spa=<?=$spa?>&m1=<?=$m1?>&m2=<?=$m2?>&m3=<?=$m3?>"><img src="/admin_page/images/btn_next02.gif" alt="다음 목록" class="btn02" /></a><?}?>
