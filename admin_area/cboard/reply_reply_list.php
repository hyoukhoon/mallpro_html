...
<script language="javascript" src="/js/popupcalendar.js"></script>
			<table width="100%" border="0" cellpadding="0" cellspacing="0">
				<tr><td class="title">대댓글 관리</td></tr>
...
<br>
			...
			<table width="100%" border="0" cellpadding="0" cellspacing="1" class="tab_01">
				<tr>
					...
					<td class="th_01" width="200">부모댓글</td>
...
//	echo $where."<br>";
$que="select count(*) from cboard_memo_memo where num>0 $where";
//echo $que."<br>";
...
$result=mysql_query("select * from cboard_memo_memo where num>0 $where order by num desc limit $t_no,$b_no") or die(mysql_error());
while($rs=mysql_fetch_object($result)){

	$result2=mysql_query("select m_content,pa_num from cboard_memo where num='$rs->pa_num'") or die(mysql_error());
	$rs2=mysql_fetch_object($result2);

	$result3=mysql_query("select subject,num from cboard where num='$rs2->pa_num'") or die(mysql_error());
...
?>
				...
				<tr>
					...
					<td class="td_02" width="200">
						<?echo content_is($rs2->m_content);?> 
					</td>
					<td class="td_02">
						<?echo content_is($rs->m_content);?> 
					</td>
					<td class="td_01" width="70">
						<?echo $rs->reg_date;?>
					</td>
					<td class="td_01" width="30"><a href="reply_reply_del.php?pa_num=<?=$rs->pa_num?>&num=<?=$rs->num?>&pa_pa_num=<?=$rs3->num?>" onclick="return confirm('삭제하시겠습니까?');">DEL</a></td>
				</tr>
<?
	$no--;
}
