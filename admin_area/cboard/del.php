...
if(!$_SESSION['MMS_ID']){
	location_is('/admin_page/login.php','','관리자만 들어올수 있습니다.');
	exit;
}
		...
		$rs = $result->fetch_object();
			...
			location_is('list.php','num='.$num.'&page_no='.$page_no.'&f_no='.$f_no.'&s_key='.$s_key.'&s_word='.$s_word.'&multi='.$multi.'&gubun='.$gubun,'삭제하였습니다.');
		}else{
			location_is('view.php','num='.$num.'&page_no='.$page_no.'&f_no='.$f_no.'&s_key='.$s_key.'&s_word='.$s_word.'&multi='.$multi.'&gubun='.$gubun,'삭제할 대상이 존재 하지 않습니다..');
		}
