<?php include $_SERVER['DOCUMENT_ROOT']."/admin_area/inc/admin_top.php";
	...
	$que="select * from proto where istate='2' and odds1>0 and (gameResult is null or gameResult='None')";
//	echo $que."<br>";
...
<tr id="tr<?=$p->gubun?>">
	<td><?=$p->iday?></td>
	  ...
	  <td><?=$p->away?></td>
	  <td>승(<?=$p->odds1?>)<input type="checkbox" odds="<?=$p->odds1?>" home="<?=$p->home?>"  away="<?=$p->away?>"  inum="<?=$p->num?>" class="chkbx" title="<?=$p->gameTitle?>" islip="<?=$p->islip?>" id="checkId[]" name="isCheck[<?=$p->islip?>]" value="1"></td>
	  <td><?if($p->odds2>0){?>무(<?=$p->odds2?>)<input type="checkbox" odds="<?=$p->odds2?>" home="<?=$p->home?>"  away="<?=$p->away?>" inum="<?=$p->num?>" class="chkbx" title="<?=$p->gameTitle?>" islip="<?=$p->islip?>" id="checkId[]" name="isCheck[<?=$p->islip?>]" value="0"><?}?></td>
	  <td>패(<?=$p->odds3?>)<input type="checkbox" odds="<?=$p->odds3?>" home="<?=$p->home?>"  away="<?=$p->away?>" inum="<?=$p->num?>" class="chkbx" title="<?=$p->gameTitle?>" islip="<?=$p->islip?>" id="checkId[]" name="isCheck[<?=$p->islip?>]" value="-1"></td>
</tr>
<?}?>  
