...
<!-- 메인메뉴 S -->
    <div id="wrap_tmall_content">
     ...
     <ul class="top_title_area">
       <li class="top_title">문의관리 > 도매회원 신청관리</li>
     </ul>
...
<input type="hidden" name="sq" id="sq"  value="<?=$sq?>">
                           ...
                           <col width="*" />
							 ...
							 <tr>
                               ...
                               <td>
									<select name="s_key">
										...
										<option value="SNAME" <?if($s_key=="SNAME"){?> selected<?}?>>상호</option>
										<option value="SELLER_NAME" <?if($s_key=="SELLER_NAME"){?> selected<?}?>>대표자</option>
							 ...
							 <tr>
                               ...
                               <td>
									...
									<input type="radio" name="STS_CODE" value="0" <?if($STS_CODE=="0"){?> checked<?}?>>처리중&nbsp;
									<input type="radio" name="STS_CODE" value="1" <?if($STS_CODE=="1"){?> checked<?}?>>처리완료&nbsp;
                              </td>
      ...
      <!-- 조회영역 E -->
<?
	$que3="select count(1) from seller_request where 1=1";
	$result3 = $mysqli->query($que3) or die("1:".$mysqli->error);
	$rs3 = $result3->fetch_array();
	$total_seller_request=$rs3[0];
	...
	if(!$order){
		$order=" order by REQUEST_SEQ desc";
	}
	...
	$que2="select count(1) from seller_request where 1=1";
	$que2.=$where;
...
?>
      ...
      <dl>
       <dt>총 게시물수 <?echo number_format($total_seller_request);?>건  |  검색결과    <?echo number_format($total);?>건</dt>
       <dd>
...
?>
                    ...
                    <tr onclick="window.open('seller_pop.php?REQUEST_SEQ=<?=$p->REQUEST_SEQ?>','r1','width=800,height=600,scrollbars=yes')" style="cursor:pointer;">
                      <td><?=$no?></td>
					  ...
					  <td><?=$p->CELLPHONE?></td>
					  <td><?if($p->STS_CODE=="1"){?>처리완료<?}else{?>처리중<?}?></td>
					  <td><?=$p->REG_DATE?></td>
                    </tr>
<?
$no--;
