...
<!-- 메인메뉴 S -->
    <div id="wrap_tmall_content">
     ...
     <ul class="top_title_area">
       <li class="top_title">문의관리 > 소매회원 신청관리</li>
     </ul>
...
<input type="hidden" name="sq" id="sq" value="<?=$sq?>">
   	       ...
   	       <table>
                           ...
                           <col width="*" />
							 ...
							 <tr>
                               ...
                               <td>
									<select name="s_key">
										...
										<option value="RNAME" <?if($s_key=="RNAME"){?> selected<?}?>>상호</option>
										<option value="RETAILER_NAME" <?if($s_key=="RETAILER_NAME"){?> selected<?}?>>대표자</option>
										<option value="BUSINESS_REGISTERED_NUMBER" <?if($s_key=="BUSINESS_REGISTERED_NUMBER"){?> selected<?}?>>사업자번호</option>
									</select>
							 ...
							 <tr>
                               ...
                               <td>
									<input type="radio" name="AUTH_CODE" value="all"  <?if($AUTH_CODE=="all"){?> checked<?}?>>전체&nbsp;
									<input type="radio" name="AUTH_CODE" value="0" <?if($AUTH_CODE=="0"){?> checked<?}?>>처리중&nbsp;
									<input type="radio" name="AUTH_CODE" value="1" <?if($AUTH_CODE=="1"){?> checked<?}?>>처리완료&nbsp;
                              </td>
      ...
      <!-- 조회영역 E -->
<?
	$que3="select count(1) from retailer where 1=1";
	...
	$que2="select count(1) from retailer where 1=1";
	$que2.=$where;
...
?>
      ...
      <dl>
       <dt>총 게시물수 <?echo number_format($total_retailer);?>건  |  검색결과    <?echo number_format($total);?>건</dt>
       <dd>
...
?>
                    ...
                    <tr onclick="window.open('retailer_pop.php?RETAILER_CODE=<?=$p->RETAILER_CODE?>','r1','width=800,height=600,scrollbars=yes')" style="cursor:pointer;">
                      <td><?=$no?></td>
                      <td class="pl20"><?=$p->RETAILER_ID?></td>
                      <td><?=$p->RNAME?></td>
                      <td><?=$p->RETAILER_NAME?></td>
					  <td><?=$p->BUSINESS_REGISTERED_NUMBER?></td>
					  <td><?=$p->CELLPHONE?></td>
					  <td><?if($p->AUTH_CODE=="1"){?>처리완료<?}else{?>처리중<?}?></td>
					  <td><?=$p->REG_DATE?></td>
                    </tr>
<?
$no--;
